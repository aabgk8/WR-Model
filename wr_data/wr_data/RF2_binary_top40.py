...
WIDE_FILE = SCRIPT_DIR / "receiving_summary_wide_by_id.csv"
MODEL_FILE = SCRIPT_DIR / f"rf_binary_top{TARGET_TOP_N}_within{TARGET_WINDOW_YEARS}yrs.joblib"
HOLDOUT_PREDICTIONS_FILE = (
    SCRIPT_DIR / f"wr_holdout_{HOLDOUT_YEAR}_binary_top{TARGET_TOP_N}_within{TARGET_WINDOW_YEARS}yrs_predictions.csv"
)
BACKTEST_FILE = SCRIPT_DIR / f"wr_binary_top{TARGET_TOP_N}_within{TARGET_WINDOW_YEARS}yrs_backtest.csv"
FEATURE_IMPORTANCE_FILE = (
    SCRIPT_DIR / f"wr_binary_top{TARGET_TOP_N}_within{TARGET_WINDOW_YEARS}yrs_feature_importance.csv"
)
...
def load_long_receiving_history(script_dir):
    ...
    files = find_raw_summary_files([script_dir, project_root])
    if not files:
...
def load_fantasy_ppg_history(script_dir):
    frames = []
    for file_path in sorted(script_dir.glob("FantasyPros_Fantasy_Football_Statistics_WR*.csv")):
