...
def merge_measurables(df, script_dir):
    """
    Merge measurable / draft-capital columns by player_id from the current source file.
    """
    candidate_files = [script_dir / "my_players_with_measurables.csv"]
    target_cols = [
        "player_id",
        "AGE_ON_DRAFT_DAY",
        "HEIGHT",
...
def main():
    ...
    pattern_wr = str(script_dir / "FantasyPros_Fantasy_Football_Statistics_WR*.csv")
    pattern_te = str(script_dir / "FantasyPros_Fantasy_Football_Statistics_TE*.csv")
    ...
    wr_te_file = script_dir / "wr_te_final.csv"
    ...
    merged_df = merge_measurables(merged_df, script_dir)
    ...
    out_file = script_dir / "wr_te_final_with_fantasy.csv"
