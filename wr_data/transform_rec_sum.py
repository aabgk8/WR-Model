...
def main():
    ...
    input_file = paths.RECEIVING_SUMMARY_WIDE_FILE
    if not input_file.exists():
        legacy_candidates = sorted(paths.PROCESSED_DIR.glob("receiving_summary_*_wide_by_id.csv"))
        if not legacy_candidates:
            print("No wide receiving summary file found. Run build_receiving_summary_wide.py first.")
            return
        input_file = legacy_candidates[-1]

    output_file = paths.TRANSFORMED_FEATURES_FILE
    max_seasons = 20
